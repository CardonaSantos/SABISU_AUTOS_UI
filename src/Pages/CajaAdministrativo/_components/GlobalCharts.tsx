...
import { FlujoGlobalDiaUI } from "../interfaces/interface2";
...
export function GlobalCharts({ data }: { data: FlujoGlobalDiaUI[] }) {
  const rows = useMemo(
    () =>
      (data ?? []).map((d) => ({
        ...
        ingresosBanco: d.ingresosTotalBanco,
        egresosBanco: d.egresosTotalBanco,
        variacionNeta: d.movimientoNetoTotal,
      })),
    [data]
  );
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      {/* Líneas: Saldos totales */}
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            Saldo total — Caja vs Banco vs Total
          </CardTitle>
        </CardHeader>
      ...
      {/* Barras + línea: flujos por canal + variación neta */}
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            Ingresos / Egresos — Caja & Banco (Global)
          </CardTitle>
        </CardHeader>
        <CardContent className="h-64">
          <ResponsiveContainer width="100%" height="100%">
