...
interface PropsEliminacion {
  data?: HistorialStockEliminacionResponse;
  isLoading: boolean;
  currentPage: number;
  totalPages: number;
  onPageChange: (page: number) => void;
  onViewDetails: (item: HistorialStockEliminacionItem) => void;
}

export default function EliminacionesTable({
  data,
  isLoading,
  currentPage,
  totalPages,
  ...
  const items: HistorialStockEliminacionItem[] = data?.data ?? [];
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center">
                  No se encontraron eliminaciones de stock.
                </TableCell>
              </TableRow>
            ) : (
              items.map((item) => (
