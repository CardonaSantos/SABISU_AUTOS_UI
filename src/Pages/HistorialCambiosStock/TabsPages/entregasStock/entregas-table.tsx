...
interface PropsEntrega {
  data?: HistorialEntregaStockResponse;
  isLoading: boolean;
  currentPage: number;
  totalPages: number;
  onPageChange: (page: number) => void;
  onViewDetails: (item: HistorialEntregaStockItem) => void;
}
...
export default function EntregasTable({
  ...
  const items: HistorialEntregaStockItem[] = data?.data ?? [];
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center">
                  No se encontraron entregas de stock.
                </TableCell>
              </TableRow>
            ) : (
              items.map((item) => (
